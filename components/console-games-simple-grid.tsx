...
function SimpleGameCardClient({ game, region, showConsole = false }: GameCardProps) {
  // Pour éviter les appels API, nous affichons seulement un placeholder
  // Les vraies images seront ajoutées plus tard via un autre système
  ...
  return (
    <div className="group relative">
      <article className="overflow-hidden rounded-lg border border-gray-200 bg-white transition-all duration-200 hover:border-gray-300 hover:shadow-sm dark:border-gray-800 dark:bg-gray-950 dark:hover:border-gray-700">
        {/* Placeholder image pour éviter les appels API */}
...
interface ConsoleGamesSimpleGridProps {
  games: GameWithConsole[]
  region: Region
  showConsole?: boolean
}

export default function ConsoleGamesSimpleGrid({ 
  games, 
  region, 
  showConsole = false 
}: ConsoleGamesSimpleGridProps) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
      {games.map((game) => (
        <SimpleGameCardClient 
          key={game.id} 
          game={game} 
          region={region}
