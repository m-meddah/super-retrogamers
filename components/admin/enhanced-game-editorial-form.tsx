...
export function EnhancedGameEditorialForm({ 
  game, 
  corporations, 
  families 
}: EnhancedGameEditorialFormProps) {
  const [isPending, startTransition] = useTransition()
  const [isPreviewMode, setIsPreviewMode] = useState(false)
  const [screenshots, setScreenshots] = useState<string[]>(game.screenshots || [])
  const [state, formAction] = useActionState(updateGameEnhancedAction, { success: false, message: '' } as { success: boolean; message: string })
  ...
  const [formData, setFormData] = useState({
    ...
    // Nouveaux champs
    corporationDevId: game.corporationDevId || '',
    corporationPubId: game.corporationPubId || '',
  ...
  const selectedCorpDev = corporations.find(c => c.id === formData.corporationDevId)
  const selectedCorpPub = corporations.find(c => c.id === formData.corporationPubId)
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-950">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              ...
              <div><strong>Taille:</strong> {formData.sizeMB ? `${formData.sizeMB} MB` : 'N/A'}</div>
            </div>
        ...
        <form action={handleSubmit} className="space-y-6">
          ...
          <div className="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-950">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label htmlFor="sizeMB" className="block text-sm font-medium text-gray-900 dark:text-white">
                  Taille (MB)
                </label>
                <input
                  type="number"
                  ...
                  onChange={(e) => handleChange('sizeMB', parseFloat(e.target.value) || '')}
                  placeholder="Ex: 256"
                  className="mt-1 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm placeholder-gray-400 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-500"
                />
              </div>
              ...
              <div>
                <label htmlFor="cloneOf" className="block text-sm font-medium text-gray-900 dark:text-white">
                  Clone de (ID du jeu original)
                </label>
                <input
                  type="text"
                  id="cloneOf"
                  name="cloneOf"
                  value={formData.cloneOf}
                  onChange={(e) => handleChange('cloneOf', e.target.value)}
                  placeholder="ID du jeu original si c'est un clone"
                  className="mt-1 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm placeholder-gray-400 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-500"
                />
              </div>
              ...
              <div className="flex items-center space-x-4">
                ...
                <div className="flex items-center">
                  <input
                    ...
                    id="onlinePlay"
                    name="onlinePlay"
                    checked={formData.onlinePlay}
                    onChange={(e) => handleChange('onlinePlay', e.target.checked)}
                    className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                  />
                  <label htmlFor="onlinePlay" className="ml-2 block text-sm text-gray-900 dark:text-white">
                    Jeu en ligne
                  </label>
                </div>
              </div>
          ...
          <div className="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-950">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Corporation Développeur */}
              <div>
                <label htmlFor="corporationDevId" className="block text-sm font-medium text-gray-900 dark:text-white">
                  Développeur (Corporation)
                </label>
                <select
                  id="corporationDevId"
                  name="corporationDevId"
                  value={formData.corporationDevId}
                  onChange={(e) => handleChange('corporationDevId', e.target.value)}
                ...
                {!formData.corporationDevId && (
                  <p className="mt-1 text-xs text-gray-500">
                    Ancien: {formData.developer || 'Aucun'}
                  </p>
                )}
              </div>
              ...
              {/* Corporation Éditeur */}
              <div>
                <label htmlFor="corporationPubId" className="block text-sm font-medium text-gray-900 dark:text-white">
                  Éditeur (Corporation)
                </label>
                <select
                  id="corporationPubId"
                  name="corporationPubId"
                  value={formData.corporationPubId}
                  onChange={(e) => handleChange('corporationPubId', e.target.value)}
                  className="mt-1 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                >
                ...
                {!formData.corporationPubId && (
                  <p className="mt-1 text-xs text-gray-500">
                    Ancien: {formData.publisher || 'Aucun'}
                  </p>
                )}
              </div>
              ...
              {/* Famille */}
              <div>
                <label htmlFor="familyId" className="block text-sm font-medium text-gray-900 dark:text-white">
                  Famille/Série
                </label>
                <select
                  id="familyId"
          ...
          <div className="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-950">
            <h3 className="text-lg font-semibold mb-4">Champs de compatibilité (legacy)</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
